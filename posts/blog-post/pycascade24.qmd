...
### Math &cup; Programming 
Presenter: Matthew Drury [](https://github.com/madrury){.bi-github}

Problem Solving by Dissolution 

Alexander Grothendieck and his student Pierre Deligne are renowned mathematicians. This talk explores their unique philosophy and approach to problem solving. 

* __Structural Development Over Strenuous Effort__:  
Problems are best solved not through strenuous effort, 
but by gradually building a structure and theory from 
which the problem naturally dissipates.

* __Simplicity in Understanding__:  Solution should be not celebrated by its difficulty but ability to be understood. 
_"I have also learned not to take glory in the difficulty of 
a proof: difficulty means we have not understood. 
The ideal is to be able to paint a landscape in which the proof is obvious." - Pierre Deligne_

* __Modeling Problem in Detail__:
Approach problems by modeling them in sufficient detail instead of writing difficult codes so that the solutions become self-evident. 

* __Data Organization__:  Even with messing coding problems, once data are organized into clean data 
structures, coding can be cleaner and more transparent.

* __Analogy of Peeling an Onion__: Messy part can be the outer layers where code is dealing with "outside world".  But the core of your code should be *well-organized* with clear data structures and self-evident operations. 

Resource: Jean-Pierre Serre, author of [A Course in Arithmatic in pdf](https://www.math.purdue.edu/~jlipman/MA598/Serre-Course%20in%20Arithmetic.pdf)) 

### Python and Rust 
Presenter: Trent H.

Best Language to write Python  is in Rust 

* demonstrated the performance bewteen Polars (in rust) vs Pandas (in python) - 11 sec and 300 sec respectively.
* Few Rust python packages: Polars, Pydantics, Ruff 
* Why Rust? Speed and ease of implmentation and distributiion 
* Rust is an unforgiving language!  
* Workflow example: 
    * write Rust code -> create and call from Python module 
* Basic idea: Argument ```FromPyObject``` into Rust, and Return ```IntoPyObject```
    * PyResult is a result layer of [Py03](https://pyo3.rs/v0.21.1/)
